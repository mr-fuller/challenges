...
def import_to_db(players=None):
    ...
    # you code ...
    	...
    	cur.execute('''CREATE TABLE players (name, year numeric, first_year numeric, team, college,
                             active numeric, games numeric, avg_min numeric, avg_points numeric)''')
...
def player_with_max_points_per_game():
    ...
    c= cur.execute('''SELECT name from players order by avg_points desc limit 1''')    
    r = c.fetchone()
    return r[0]
...
def percentage_of_players_first_year():
    ...
    r = c.fetchone()
    first_yr = r[0]
    # print(first_yr, type(first_yr))
    c = cur.execute('''Select count(*) from players''')
    r = c.fetchone()
    total = r[0]
    # print(total, type(total))
    print( first_yr / total * 100.0)
    return float(first_yr) / float(total) * 100.0
